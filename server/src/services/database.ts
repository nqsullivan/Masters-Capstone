...
class DatabaseAccess {
  ...
  private async connect() {
    ...
    await this.connection.run(
      `CREATE TABLE IF NOT EXISTS attendance (id VARCHAR, studentId VARCHAR, sessionId VARCHAR, checkIn DATETIME default NULL, portraitUrl VARCHAR, portraitCaptured BOOLEAN, FRIdentifiedId VARCHAR, status VARCHAR, flagged BOOLEAN default FALSE, videoKey VARCHAR(2083))`
    );
    await this.connection.run(
